...
in this particular dataset. It's also interesting that most of the wines in this 
dataset do have traces of citric acid even though these are all red wines.
...
rw_copy <- cbind(rw)
# convert the 'quality' variable from categorical to numeric so R will be able to do correlation coefficients
rw_copy$quality <- as.numeric(levels(rw_copy$quality))[as.integer(rw_copy$quality)]
# check t make sure conversion was made
# str(rw_copy)
...
The greatest correlations between acidity and quality are with volatile acidity 
(negative correlation - medium) and citric acid (positive correlation - weak). Volatile
acidity is due to aceitic acid which at high quantities gives the taste of [balsamic
...
more surprising. We'll explore both of these items further.
...
Let's examine another theoretically related pair of variables: alchohol and residual.sugar. Since alcohol in wine is created by chemical reaction with sugar that happens during
fermentation, our hypothesis is the higher the level of alcohol, the lower the amount
of residual sugar.

First let's subset the dataframe:
```{r}
rw_sugalc  <-  rw_copy[ ,c('quality', 'alcohol', 'residual.sugar')]
rw_sugalc$quality_factor <- as.factor(rw_sugalc$quality)
```

Now let's view a scatterplot of (a subset of) residual sugar vs. alchohol (faceted by quality).

```{r}
ggplot(data = subset(rw_sugalc, quality > 6 | quality < 5), aes(y = residual.sugar, x = alcohol, color = quality_factor)) +
  geom_point() +
  scale_color_brewer()+
...
```
Hypothesis here appears to be incorrect since the correlation coefficient is very small.

NEED TO LOOK UP REASON OR HAVE SOME EXPLANATION
...
rw_goodbad <- select(filter(rw_copy, quality > 6 | quality < 5), c(id,quality,
                                                                   volatile.acidity, 
                                                                   citric.acid,residual.sugar,
                                                                   chlorides, sulphates, 
                                                                   alcohol))
# using brackets in combination with the which() function and the %in% operator
ed_exp3 <- education[which(education$Region == 2),names(education) %in% c("State","Minor.Population","Education.Expenditures")]
# using subset function - not recommended
rw_goodbad <- subset(rw_copy, quality > 6 | quality < 5)

ggpairs (rw_goodbad, columns = 2:8, axisLabels = 'internal')
dim(rw_goodbad)
cor(rw_goodbad$alcohol,rw_goodbad$residual.sugar)
rw_goodbad_acid <- rw_goodbad[ ,c('quality', 'pH', 'total.acidity', 'fixed.acidity', 'volatile.acidity', 'citric.acid')]
ggpairs(rw_goodbad_acid, axisLabels = 'internal')
...
```
```{r}
# subsetting dataframe keying on most relevant variables
rw_subset = rw_copy[ ,c('quality', 'alcohol', 'pH', 'residual.sugar', 'total.sulfur.dioxide')]
# names(rw_subset)
str(rw_subset)
rw_subset_num <- rw_subset
rw_subset_num$quality <- as.numeric(rw_subset_num$quality)
rw_subset_fact <- rw_subset
str(rw_subset_fact)
str(rw_subset_num)
